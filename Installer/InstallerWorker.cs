...
namespace Installer
{
    public class InstallerWorker
    {
        ...
        public void Run()
        {
            ...
            HasError = !InstallerFunctions.CheckFSUIPC();
            if (HasError)
                messageList.Enqueue($"Error: FSUIPC not installed our outdated! Minimum Version: {Parameters.ipcVersion}");
            ...
            messageList.Enqueue("\nDone.");
            if (!HasError)
                messageList.Enqueue($"WorkingTitle2GSX was installed to {Parameters.appDir}");
            IsRunning = false;
        }
        ...
        protected void InstallApp()
        {
            ...
            {
                messageList.Enqueue("Installing WorkingTitle2GSX ...");
            ...
            if (!File.Exists(Parameters.confFile))
            {
                ...
                using (var resource = Assembly.GetExecutingAssembly().GetManifestResourceStream("Installer.WorkingTitle2GSX.config"))
                {
